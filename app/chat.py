    ...
    try:
        ...
        with st.spinner("🤖 Generating response..."):
            ...
            response = llm_client.generate_response(
                ...
                context_documents=context_documents,  # consistent list[str] from vector store
                user_role=user_role,
...
def process_user_query_stream(user_query: str):
    """Yield streaming response chunks for the given query."""
    user_role = get_current_user_role()
    if not user_role:
        yield "Error: Unable to determine user role. Please login again."
        return
    ...
    if not vector_store or not vector_store.collection:
        yield "⚠️ Knowledge base is not available. Please contact administrator."
        return
    if not llm_client:
        yield "⚠️ AI service is not available. Please try again later."
        return
    ...
    try:
        ...
        if not context_documents:
            yield f"We couldn’t find relevant information for your role (**{user_role}**) on this query. Please try rephrasing or ask a different question."
            return


        # Stream LLM response with context
        stream = llm_client.generate_streaming_response(
            user_query=user_query,
            ...
            ] if len(st.session_state.chat_history) > 1 else None,
            max_tokens=1024,
            temperature=0.7
        )
        for chunk in stream:
        ...
        st.error(f"Error processing streaming query: {e}")
        yield "I’m sorry, we encountered an error while preparing the response. Please try again in a moment."
